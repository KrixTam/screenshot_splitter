...
*   **双轮梳理机制**：
    ...
    2.  **二次优化**：针对首轮中未发生合并的“孤立区块”，将其与新的邻居（可能是第一轮产生的合并块）再次进行语义比对，确保复杂的 UI 组件能被完整地聚合在一起。
...
```json
{
  ...
  "results": {
    "blocks": [...],           // 有效内容块元数据
    "invalidBlocks": [...],    // 无效/太小的内容块元数据
    "separators": [...],       // 分割背景区元数据
    "completeness": 95,        // 物理拆解覆盖度 (%)
    "refinedBlocks": [...],    // 智能梳理后的块元数据 (可选)
    "refinementCompleteness": 98 // 智能梳理度 (%)
  }
}
```

## 使用场景
- **UI 组件库构建**：快速将长截图拆解为原子级或组件级图片素材。
- **页面逻辑分析**：可视化分析页面组件的垂直堆叠逻辑与语义关系。
- **快速恢复与共享**：导出完整分析状态，方便跨设备继续执行梳理操作。
